...
class CamPublishStream : public rclcpp::Node
{
public:
  RCLCPP_SMART_PTR_DEFINITIONS(CamPublishStream)

  explicit CamPublishStream(const rclcpp::NodeOptions & options , GstCamConfig *camcfg);
  ~CamPublishStream();
  int32_t setZoom(uint32_t size);
